...
if(!empty($_POST)){
    ...
    if(empty($_POST['pseudo']) || empty($_POST['mdp'])) {/* Je vérifie que pseudo et mdp contiennent des infos */
    ...
    if(empty($contenu)){/* si la variable contenu est vide alors je n'ai pas d'erreurs ! */
        ...
        $resultat->execute(array(
            ':pseudo' => $_POST['pseudo'],
        )); /* je sélectionne dans ma bdd le membre dont le pseudo correspond à celui du formulaire */

        if($resultat->rowCount() == 1){/* si le programme renvoie une ligne c'est que ce membre et ce pseudo existent ! */
            $membre = $resultat->fetch(PDO::FETCH_ASSOC);

            if(password_verify($_POST['mdp'], $membre['mdp'])){
                /* password_verify prend deux arguments : le premier est ce que l'on récupère dans le formulaire. Le second est le mdp hashé qui existe dans la BDD. password_verify vérifie que le premier correspond au deuxième. */
                $_SESSION['membres'] = $membre;/* on crée une session avec les infos du membre dans un tableau multidimensionnel */
